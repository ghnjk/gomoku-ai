...
class SelfPlayTranner(object):
    ...
    def __init__(self, rowCount, colCount, nInRow, trainBatchCnt, gameCntPerBatch
                 , trainStoreFileName, ossPath, workerCount = 8, trainBatchPerIter = 4):
        """
        trainBatchCnt: 训练批次数
    ...
    def calcMctsPlayout(self):
        ...
        if blackWin > whiteWin:
            blackWin -= whiteWin
            whitePlayout += (blackWin / 5) * 50
            blackPlayout -= (blackWin / 5) * 25
        else:
            whiteWin -= blackWin
            blackPlayout += (whiteWin / 5) * 50
            whitePlayout -= (whiteWin / 5) * 25
    ...
    def uploadToOss(self):
        self.syncFile("model.json")
        self.syncFile("weight.hdf5", remoteFileName = "weight_%d.hdf5" % (self.selfPlayCnt))
        if self.curPlayIter % 20:
            self.syncFile("train_data.db")

    def syncFile(self, fileName, remoteFileName = None):
        localPath = os.path.join("data", fileName)
        if remoteFileName is None:
            remoteFileName = fileName
        remotePath = os.path.join(self.ossPath, remoteFileName)
        with open(localPath, "rb") as fpSrc:
            fp = gfile.FastGFile(remotePath, "wb")
            fp.write(fpSrc.read())
            fp.close()
...
if __name__ == '__main__':
    trainner = SelfPlayTranner(
        ...
        , workerCount = 5
        , trainBatchPerIter = 5
        , ossPath = "oss://jk-deep-learning-backet.oss-cn-beijing-internal.aliyuncs.com/gomoku_ai/data"
    )
    trainner.run()
